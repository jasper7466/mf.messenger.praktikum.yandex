    ...
    describe('Black Box Tests', () => {
        ...
        describe('Метод delete(path)', () => {
            ...
            it('Удаление объекта по несуществующему пути. Ожидается исключение', () => {
                store.set('a.b.c', { d: 123, e: 456 });
                expect(() => store.delete('a.b.c.f.g')).to.throw(
                    `${store.constructor.name}: Key "f" of path "a.b.c.f" doesn't exist in store`
                );
            });
    ...
    describe('White Box Tests', () => {
        ...
        describe('Метод _getByPathOrRaiseError(path)', () => {
            ...
            it('Выброс исключения при получении значения по несуществующему пути', () => {
                expect(() => getByPathOrRaiseError('non.existed.path')).to.throw(
                    `${store.constructor.name}: Key "non" of path "non.existed.path" doesn't exist in store`
                );
            });
        });
