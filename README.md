...
> Реализовано:
> - Класс для работы с HTTP (скорее всего не совсем корректно, т.к. не получается выгрузить аватар на сервер, уходит пустой запрос без FormData)
> - Роутинг (отрабатывает корректно события "вперёд/назад", но обновление страницы всё ломает)
> - API чатов (пока не все, только требуемые для выполнения текущего задания)
> 
> Не реализовано
> - Модульные тесты
> - Защита XSS/DOS
...
> 
> Известные проблемы
> - Почему-то не отправляется FormData в запросе на обновление аватара
> - Не получается подгружать аватары, хотя ссылки формата "/path/to/avatar" приходят, но после подстановки к ним baseURL - они не открываются
> - Хранилище не дописано, не реализована очистка
> - Запрос на добавление пользователя в чат не отрабатывает корректно, хотя передаются существующие id пользователя и чата
> - Запрос на удаление пользователя из чата всегда отвечает "200 OK" (возможно, это нормально)
> - Компонент "кнопка", возможно, реализован не очень правильно. По сути он возвращает только разметку и рендерится во вью как текст. В прошлом спринте было требование использовать его для всех кнопок, но это неудобно - обработчики событий на неё не забиндить и приходится вешать один общий обработчик на всю страницу.
> 
> P.S.
> 
> Есть стойкое ощущение, что помимо всего прочего, архитектурно проект реализован не очень хорошо, не так как оно задумано.
> Буду рад любым замечаниям и советам, которые помогут сделать его лучше.
> 
> Спасибо!
